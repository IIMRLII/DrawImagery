package com.example.drawimagery;

import android.content.Context;
import android.graphics.Bitmap;
import android.graphics.BitmapFactory;
import android.graphics.BitmapShader;
import android.graphics.Canvas;
import android.graphics.Color;
import android.graphics.ComposeShader;
import android.graphics.LinearGradient;
import android.graphics.Paint;
import android.graphics.PorterDuff;
import android.graphics.Rect;
import android.graphics.RectF;
import android.graphics.Shader;
import android.graphics.SweepGradient;
import android.util.AttributeSet;
import android.view.MotionEvent;
import android.view.View;

import androidx.annotation.Nullable;

public class ColorPickerView extends View {
    private static final float PI = (float) Math.PI;

    Bitmap transparentBitmap = null;

    private Paint paintCirclePhantom;
    private Paint paintCircle;
    private Paint paintCenterShadow;
    private Paint paintCenter;
    private Paint paintGrayShadow;
    private Paint paintGray;
    private Paint paintTransparent;
    BitmapShader mBitmapShader = null;
    Shader mComposeShader = null;

    private float mouseCurrentX;
    private float mouseCurrentY;
    private Paint mousePaint;

    private Paint paintLightShadow;
    private Paint paintLight;
    private double Zoom;
    private int[] arrColorGray;
    private int[] arrColorTransparent;
    private final int[] arrColorCircle = new int[]{0xFFFF0000, 0xFFFF00FF,
            0xFF0000FF, 0xFF00FFFF, 0xFF00FF00, 0xFFFFFF00, 0xFFFF0000};
    private boolean mRedrawHSV;
    private boolean mRedrawTransparent;

    private int CenterX = 100;
    private int CenterY = 100;
    private int CenterRadius = 30;
    private String strColor = "";

    private OnColorBackListener l;

    public ColorPickerView(Context context) {
        super(context);
        pick_init();
    }

    public ColorPickerView(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        pick_init();
    }

    public ColorPickerView(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        pick_init();
    }

    private void pick_init() {
        float density = getContext().getResources().getDisplayMetrics().density;
        double Zoom = (double) (density / 2.0 + 0.5);
        int color = Color.parseColor("#FFFFFFFF");
        init(color, Zoom);
    }

    public ColorPickerView(Context context, int color, double Zoom) {
        super(context);
        init(color, Zoom);
    }

    private void init(int color, double Zoom) {
        this.Zoom = Zoom;
        transparentBitmap = BitmapFactory.decodeResource(getResources(),R.drawable.transparent);
        CenterX = (int) (100 * Zoom);
        CenterY = (int) (100 * Zoom);
        mouseCurrentX = CenterX;
        mouseCurrentX = CenterY;
        mousePaint = new Paint(Paint.ANTI_ALIAS_FLAG);
        mousePaint.setStrokeWidth(2);
        mousePaint.setStyle(Paint.Style.STROKE);

        CenterRadius = (int) (30 * Zoom);
        paintCirclePhantom = new Paint(Paint.ANTI_ALIAS_FLAG);
        paintCircle = new Paint(Paint.ANTI_ALIAS_FLAG);
        paintCenterShadow = new Paint(Paint.ANTI_ALIAS_FLAG);
        paintCenter = new Paint(Paint.ANTI_ALIAS_FLAG);
        paintGrayShadow = new Paint(Paint.ANTI_ALIAS_FLAG);
        paintGray = new Paint(Paint.ANTI_ALIAS_FLAG);
        paintTransparent = new Paint(Paint.ANTI_ALIAS_FLAG);
        paintLightShadow = new Paint(Paint.ANTI_ALIAS_FLAG);
        paintLight = new Paint(Paint.ANTI_ALIAS_FLAG);
        paintCirclePhantom.setColor(0xFF000000);
        paintCirclePhantom.setStyle(Paint.Style.STROKE);
        paintCirclePhantom.setStrokeWidth((float) (32 * Zoom));

        paintCircle.setShader(new SweepGradient(0, 0, arrColorCircle, null));
        paintCircle.setStyle(Paint.Style.STROKE);
        paintCircle.setStrokeWidth((float) (32 * Zoom));

        paintCenterShadow.setColor(0xFF000000);
        paintCenterShadow.setStyle(Paint.Style.STROKE);
        paintCenterShadow.setStrokeWidth((float) (2 * Zoom));

        paintCenter.setColor(color);
        paintCenter.setStrokeWidth((float) (5 * Zoom));

        paintGrayShadow.setColor(0xFF000000);
        paintGrayShadow.setStrokeWidth((float) (30 * Zoom));

        arrColorGray = new int[]{0xFFFFFFFF, color, 0xFF000000};
        paintGray.setStrokeWidth((float) (30 * Zoom));

        arrColorTransparent = new int[]{0x00FFFFFF, color, 0xFFFFFFFF};
        paintTransparent.setStrokeWidth((float) (30 * Zoom));

        mBitmapShader = new BitmapShader(transparentBitmap, Shader.TileMode.REPEAT, Shader.TileMode.REPEAT);

        paintLightShadow.setColor(0xFF000000);
        paintLightShadow.setStrokeWidth((float) (60 * Zoom));

        paintLight.setStrokeWidth((float) (60 * Zoom));

        mRedrawHSV = true;
        mRedrawTransparent = true;
    }

    @Override
    protected void onDraw(Canvas canvas) {
        canvas.translate(CenterX, CenterY);
        float r = CenterX - paintCircle.getStrokeWidth() * 0.5f;
        int color = paintCenter.getColor();
        strColor = "#" + Integer.toHexString(color).substring(2).toUpperCase();

        if (mRedrawHSV) {
            arrColorGray[1] = color;
            paintGray.setShader(new LinearGradient(CenterX, -CenterY, CenterX,
                    (float) (100 * Zoom), arrColorGray, null,
                    Shader.TileMode.CLAMP));
        }

        canvas.drawOval(new RectF(-r + 3, -r + 3, r + 3, r + 3),
                paintCirclePhantom);
        canvas.drawOval(new RectF(-r, -r, r, r), paintCircle);
        paintCenterShadow.setAlpha(paintCenter.getAlpha());
        canvas.drawCircle(2, 2, CenterRadius - 1, paintCenterShadow);
        canvas.drawCircle(0, 0, CenterRadius, paintCenter);
        canvas.drawRect(new RectF(CenterX + (float) (23 * Zoom), -CenterY + 3,
                        CenterX + (float) (53 * Zoom), (float) (103 * Zoom)),
                paintGrayShadow);
        canvas.drawRect(new RectF(CenterX + (float) (20 * Zoom), -CenterY,
                CenterX + (float) (50 * Zoom), (float) (100 * Zoom)), paintGray);
        canvas.drawRect(new RectF(CenterX + (float) (68 * Zoom), -CenterY + 3,
                        CenterX + (float) (98 * Zoom), (float) (103 * Zoom)),
                paintGrayShadow);

        LinearGradient mLinearGradient = new LinearGradient(CenterX, -CenterY, CenterX, CenterY, new int[] {
                Color.WHITE, color}, null,
                Shader.TileMode.CLAMP);
        mComposeShader = new ComposeShader(mBitmapShader, mLinearGradient, PorterDuff.Mode.MULTIPLY);
        if (mRedrawTransparent) paintTransparent.setShader(mComposeShader);
        RectF dst = new RectF(CenterX + (float) (65 * Zoom), -CenterY,//透明区域
                CenterX + (float) (95 * Zoom), (float) (100 * Zoom));
        canvas.drawRect(dst, paintTransparent);

        //十字准星
        int current_color = paintCenter.getColor();
        mousePaint.setColor(Color.argb(255, 255 - Color.red(current_color), 255 - Color.green(current_color), 255 - Color.blue(current_color)));
        canvas.drawLine((float) (-100 * Zoom), mouseCurrentY, CenterX * 2 + 200, mouseCurrentY, mousePaint);
        canvas.drawLine(mouseCurrentX, (float) (-100 * Zoom), mouseCurrentX, CenterY * 2 + 10, mousePaint);
        canvas.drawCircle(mouseCurrentX, mouseCurrentY, (float) (10 * Zoom), mousePaint);
        canvas.drawCircle(mouseCurrentX, mouseCurrentY, (float) (5 * Zoom), mousePaint);

        mRedrawHSV = true;
        mRedrawTransparent = true;
    }

    @Override
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        setMeasuredDimension(CenterX * 2 + 200, CenterY * 2 + 10);
    }

    private int ave(int s, int d, float p) {
        return s + java.lang.Math.round(p * (d - s));
    }

    private int interpColor(int colors[], float unit) {
        if (unit <= 0) {
            return colors[0];
        }
        if (unit >= 1) {
            return colors[colors.length - 1];
        }

        float p = unit * (colors.length - 1);
        int i = (int) p;
        p -= i;

        int c0 = colors[i];
        int c1 = colors[i + 1];
        int a = ave(Color.alpha(c0), Color.alpha(c1), p);
        int r = ave(Color.red(c0), Color.red(c1), p);
        int g = ave(Color.green(c0), Color.green(c1), p);
        int b = ave(Color.blue(c0), Color.blue(c1), p);
        if (l != null) {
            l.onColorBack(a, r, g, b);
        }
        return Color.argb(a, r, g, b);
    }

    @Override
    public boolean onTouchEvent(MotionEvent event) {
        float x = event.getX() - CenterX;
        float y = event.getY() - CenterY;
        boolean inCenter = java.lang.Math.sqrt(x * x + y * y) <= CenterRadius;

        mouseCurrentX = x;
        mouseCurrentY = y;

        if(y < -100 * Zoom || y > 100 * Zoom) return false;
        switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN: {

            }
            case MotionEvent.ACTION_MOVE: {
                if ((x >= -CenterX && x <= CenterX)
                        && (y >= -CenterY && y <= CenterY)) {//色环选色
                    float angle = (float) java.lang.Math.atan2(y, x);
                    float unit = angle / (2 * PI);
                    if (unit < 0)
                        unit += 1;
                    paintCenter.setColor(interpColor(arrColorCircle, unit));
                    invalidate();
                } else if (x >= CenterX + 20 * Zoom && x <= CenterX + 50 * Zoom) {//饱和度
                    int a, r, g, b, c0, c1;
                    float p;

                    if (y < 0) {
                        c0 = arrColorGray[0];
                        c1 = arrColorGray[1];
                        p = (float) ((y + 100 * Zoom) / (100 * Zoom));
                    } else {
                        c0 = arrColorGray[1];
                        c1 = arrColorGray[2];
                        p = (float) (y / (100 * Zoom));
                    }

                    a = ave(Color.alpha(c0), Color.alpha(c1), p);
                    r = ave(Color.red(c0), Color.red(c1), p);
                    g = ave(Color.green(c0), Color.green(c1), p);
                    b = ave(Color.blue(c0), Color.blue(c1), p);

                    paintCenter.setColor(Color.argb(a, r, g, b));
                    mRedrawHSV = false;
                    if (l != null) {
                        l.onColorBack(a, r, g, b);
                    }
                    invalidate();
                } else if (x >= CenterX + 65 * Zoom && x <= CenterX + 95 * Zoom) {//透明度
                    float p;
                    if (y < 0) {
                        p = (float) ((y + 100 * Zoom) / (200 * Zoom));
                    } else {
                        p = (float) (y / (200 * Zoom)) + 0.5F;
                    }

                    int alpha = (int) (255 * p);

                    paintCenter.setAlpha(alpha);
                    mRedrawHSV = false;
                    mRedrawTransparent = false;
                    int paintColor = paintCenter.getColor();
                    if (l != null) {
                        l.onColorBack(alpha, Color.red(paintColor), Color.green(paintColor), Color.blue(paintColor));
                    }
                    invalidate();
                }
                break;
            }
            case MotionEvent.ACTION_UP: {

            }
        }
        return true;
    }

    public void setOnColorBackListener(OnColorBackListener l) {
        this.l = l;
    }

    public interface OnColorBackListener {
        public void onColorBack(int a, int r, int g, int b);
    }
}
